...
function getGoalProgress(goalId) {
  ...
  const pct = Math.round((completed / goalTasks.length) * 100);
  return { total: goalTasks.length, completed, pct };
}
  ...
  container.innerHTML = allGoals.map(goal => {
    ...
    <div class="goal-card" data-id="${goal.id}">
      ...
      <div class="task-actions" style="margin-top:12px;">
        <button class="btn btn-danger" style="padding:8px 12px;font-size:13px;margin-left:auto;" data-action="delete-goal" data-id="${goal.id}">
          ${icon('x', 13)}
        </button>
      </div>
    </div>`;
...
function renderAchievements() {
  ...
  if (!allAchievements.length) {
    container.innerHTML = `
      <div class="empty-state" style="grid-column:1/-1">
        <div class="empty-icon">${icon('trophy', 40)}</div>
        <h3>No achievements found</h3>
        <p>Complete tasks and goals to start unlocking achievements.</p>
      </div>`;
    return;
  }
...
function renderGoalTaskPicker() {
  ...
  if (!available.length) {
    container.innerHTML = `<p style="color:rgba(255,255,255,0.3);font-size:13px;padding:8px 0;">
      No unassigned tasks available. Create some tasks first.
    </p>`;
    return;
  }
  container.innerHTML = available.map(t => `
    ...
    ">
      <input type="checkbox" name="goal-tasks" value="${t.id}"
        style="width:auto;accent-color:var(--orange);"
        onchange="updateGoalTaskCount()">
      <span>${escapeHtml(t.title)}</span>
      ${getCategoryName(t.category_id)
        ? `<span style="margin-left:auto;font-size:11px;color:rgba(255,255,255,0.3);">${escapeHtml(getCategoryName(t.category_id))}</span>`
        : ''}
    </label>
  `).join('');
}
...
function updateGoalTaskCount() {
  ...
  if (counter) {
    ...
    counter.style.color = checked >= 3 ? 'var(--orange)' : 'rgba(255,255,255,0.3)';
  }
  if (submitBtn) {
    submitBtn.disabled = checked < 3;
    submitBtn.title    = checked < 3 ? 'Select at least 3 tasks' : '';
  }
}
...
async function loadAchievementCount() {
  try {
    ...
    console.error('Failed to load achievement count:', err.message);
  }
}
...
async function updateTaskStatus(taskId, status) {
  try {
    ...
    if (status === 'completed' && task?.goal_id) {
      await checkGoalCompletion(task.goal_id);
    }
...
async function addGoal(e) {
  ...
  const selectedIds = [...document.querySelectorAll('input[name="goal-tasks"]:checked')]
    .map(cb => cb.value);

  if (!title) return;
  if (selectedIds.length < 3) {
    toast('Select at least 3 tasks for this goal', 'error');
    return;
  }
  ...
  try {
    ...
    await Promise.all(selectedIds.map(taskId =>
...
async function checkGoalCompletion(goalId) {
  ...
  try {
    const data = await apiFetch(`/goals/${goalId}`, {
      method: 'PUT',
    ...
    if (data?.pointsAwarded) {
      ...
      toast(`Goal "${goal.title}" complete!`, 'success');
    }
    ...
    console.error('Goal completion failed:', err.message);
  }
}
...
async function deleteGoal(goalId) {
  try {
    const goalTasks = allTasks.filter(t => t.goal_id === goalId);
    await Promise.all(goalTasks.map(t =>
      apiFetch(`/tasks/${t.id}`, {
        method: 'PUT',
